...
import com.jd.sponge.util.Utilities;
	...
	public class SpongeArrayBlockingQueue<E> extends AbstractQueue<E>
	    ...
	    class MemoryItemList
		{
			...
			{
				System.out.println("==========generateOneBatchBytes==========向持久化文件中写入文件!!!!!!!!!!!!!!!");
				theBytesOut.reset();
				//将theBytesOut的pos设置在6的位置
				...
				try
				{
					...
					tmpData[0] = 7;
					tmpData[1] = 7;
					//填充theBytesOut的其他位置 2,3,4,5  这样就能填充完setSize中的前6个位置
					//theBytesOut的前两个字节是magic便签, 2,3,4,5这4个字节是一个int值，代表的是字节数组的长度
			...
			private void fetchData_init(ThreadPoolExecutor theThreadPoolExecutorInsParm)
			{
				if (theSpongeService.getThePersistence().isHaveDataInPersistence() == true)
				{
					try
					{
						System.out.println("=========fetchData_init======从持久化中取数据!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			...
			private boolean fetchData()
			{
				...
				try
				{
					...
					{
						System.out.println("=================fetchData=========开始从持久化中取数据!!!!!!!!!!!!!!!!!!!!!!!!!");
						byte[] tmpBytes = theSpongeService.getThePersistence().fetchOneBatchBytes();
						
						if (tmpBytes != null)
						{
							long tmpStartTime = System.currentTimeMillis();
							int tmpCurPosi = 6;//在写入的时候(见队列的offer方法) 前6个字节写入的是其他内容
							while (tmpCurPosi < tmpBytes.length)
							{
								int tmpOneItemLength = Utilities.getIntFromBytes(tmpBytes, tmpCurPosi);
								tmpCurPosi += 4;
