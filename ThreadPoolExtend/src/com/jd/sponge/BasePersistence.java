...
public abstract class BasePersistence implements PersistenceIntf
{
	...
	private long isCanReleaseResMaxTime = 15 * 1000;
	...
	public boolean addOneBatchBytes(byte[] bytesParm)
	{
		...
		if (isCanReleaseResTime != -1)
        {
        					...
        					> isCanReleaseResMaxTime)
        	{
        		try
        		{
					//会先判断下当前文件是不是需要先释放掉    现在的逻辑是如果超过60s没数据进入的话，再有新数据进入时会删除掉之前的持久化文件
        			canReleaseRes();
        		}
        		catch (Exception ex)
        		{
	...
	public byte[] fetchOneBatchBytes() throws SpongeException
	{
		...
		try
		{
			synchronized(WriteAndReadMutex)
			{
				...
				System.out.println(retBytes==null ? "是null":"false");
				if (retBytes == null)//当取到的字节数组为空时,会将isCanReleaseResTime置为当前时间
				{
